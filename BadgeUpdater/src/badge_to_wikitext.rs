...
use crate::{
    clean_badge_name,
    definitions::{Badge, ProcessError, RobloxBadgeData},
...
pub async fn get_badges_wiki(
    ...
    url: &Url,
    ignore: &[u64],
) -> Result<Vec<Result<Vec<Result<WikiText, ProcessError>>, ProcessError>>, ProcessError> {
    ...
    while let Some(next_page_cursor) = data.next_page_cursor {
        ...
        let filter = data
            ...
            .collect_vec();
        for process in filter.chunks(50) {
            results.push(get_wiki_pages(client, process).await);
        }
    }
    Ok(results)
}
...
pub async fn get_wiki_pages(
    client: &RustClient,
    badges: &[Badge],
) -> Result<Vec<Result<WikiText, ProcessError>>, ProcessError> {
    let page_data = get_pages(client, &badges.iter().map(|b| b.name.clone()).collect_vec()).await?;
    let mut results = vec![];
    ...
    if let Some(ref pages) = page_data.query.pages {
        for page in pages {
                ...
                && miss
            {
                ...
                searches.push(badges.iter().find(|b| b.name == page.title).unwrap());
                continue;
            }
    ...
    for search in searches {
        ...
        if let Some(searches) = search_data.query.search {
            ...
            for page in pages.query.pages.unwrap() {
                ...
                if content.contains(&search.id.to_string()) {
                    let mut wt = WikiText::parse(content);
...
fn get_redirect<S: AsRef<str>>(content: &S) -> Option<&str> {
    // according to <https://www.mediawiki.org/wiki/Help:Redirects>, redirects can only be line 1.
    let line = content.as_ref().lines().next().unwrap();

    let (_, redirect) = lazy_regex::regex_captures!(r"(?mi)#redirect \[\[(.+)\]\]", &line)?;
    Some(redirect)
}
